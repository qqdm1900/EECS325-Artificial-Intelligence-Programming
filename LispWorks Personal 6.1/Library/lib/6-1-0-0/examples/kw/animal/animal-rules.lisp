;; -*- Mode: Lisp; rcs-header: "$Header: /hope/lwhope1-cam/hope.0/compound/58/KWdemos/RCS/animal:animal-rules.lisp,v 1.1.13.1 2011/08/24 13:25:20 davef Exp $" -*-
...
(defrule animal-question :forward
  ...
  -->
  ((capi:confirm-yes-or-no "Is it a ~a?" ?animal) ?answer)
  (erase ?current)
  (assert (game-over ? node ?node animal ?animal answer ?answer)))
...
(defrule game-finished :forward
  ...
  -->
  (erase ?g)
;  (test (not (capi:confirm-yes-or-no "Play again?")))
  (return))
